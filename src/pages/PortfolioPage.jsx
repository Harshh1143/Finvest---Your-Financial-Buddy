...
import { Search, Sparkles, Plus, Trash2, Pencil, ArrowUpRight, ArrowDownRight, } from "lucide-react";
...
export function PortfolioPage() {
    ...
    const searchResults = useMemo(() => {
        ...
        if (!q)
            return Object.keys(DEFAULT_ASSETS_INFO);
        return Object.keys(DEFAULT_ASSETS_INFO).filter((ticker) => ticker.includes(q) ||
    ...
    return (<Shell>
      <div className="mx-auto max-w-7xl px-6 py-10 lg:px-8">
        <div className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
          ...
          <Button onClick={handleAutoRefresh} disabled={portfolio.length === 0} className="w-fit py-5 rounded-2xl font-semibold bg-cyan-400 hover:bg-cyan-500 text-slate-950 disabled:opacity-50">
          ...
          <Card className="border-white/10 bg-slate-950/65 overflow-hidden relative">
            ...
            <CardContent className="space-y-4">
              <label className="flex items-center gap-3 rounded-2xl border border-white/10 bg-white/5 px-4 py-3 text-sm text-slate-400 focus-within:border-cyan-400/50 transition">
                ...
                <input value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} className="w-full bg-transparent text-sm text-white outline-none placeholder-slate-500" placeholder="Search popular equities, cryptocurrencies..."/>
              </label>

              <div className="grid gap-2 max-h-56 overflow-y-auto pr-1">
                {searchResults.map((ticker) => {
            const asset = DEFAULT_ASSETS_INFO[ticker];
            return (<button key={ticker} type="button" onClick={() => setSelectedTicker(ticker)} className={`flex items-center justify-between rounded-xl border p-3.5 text-left transition ${selectedTicker === ticker
            ...
            setPurchasePrice("");
        }} className={`flex items-center justify-between rounded-xl border p-3.5 text-left transition ${selectedTicker === "CUSTOM"
            ? "border-cyan-400/40 bg-cyan-500/10 text-white"
            : "border-white/5 bg-white/5 text-slate-300 hover:bg-white/10"}`}>
                  <div>
              ...
              {selectedTicker === "CUSTOM" && (<motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="space-y-3 rounded-2xl border border-white/5 bg-white/5 p-4 mt-2">
                  ...
                  <div className="grid gap-3 sm:grid-cols-2">
                    <label className="text-xs text-slate-400 space-y-1 block">
                      ...
                      <input type="text" placeholder="e.g. AMZN, GOLD" value={customTicker} onChange={(e) => setCustomTicker(e.target.value)} className="w-full rounded-xl border border-white/10 bg-slate-900 px-3 py-2 text-white outline-none"/>
                    </label>
                    <label className="text-xs text-slate-400 space-y-1 block">
                      <span>Asset Class</span>
                      <select value={customType} onChange={(e) => setCustomType(e.target.value)} className="w-full rounded-xl border border-white/10 bg-slate-900 px-3 py-2 text-white outline-none">
                  ...
                  <label className="text-xs text-slate-400 space-y-1 block">
                    ...
                    <input type="text" placeholder="e.g. Amazon.com Inc." value={customName} onChange={(e) => setCustomName(e.target.value)} className="w-full rounded-xl border border-white/10 bg-slate-900 px-3 py-2 text-white outline-none"/>
                  </label>
                </motion.div>)}
            </CardContent>
          </Card>
            ...
            <CardContent>
              <form onSubmit={handleAddAsset} className="space-y-4">
                ...
                <div className="grid gap-3 sm:grid-cols-2">
                  <label className="text-xs text-slate-400 space-y-1 block">
                    <span>Quantity</span>
                    <input type="number" step="0.00001" className="w-full rounded-xl border border-white/10 bg-white/5 px-3 py-2.5 text-white outline-none focus:border-cyan-400" value={quantity} onChange={(e) => setQuantity(e.target.value)} required/>
                  </label>
                  <label className="text-xs text-slate-400 space-y-1 block">
                    <span>Purchase date</span>
                    <input type="date" className="w-full rounded-xl border border-white/10 bg-white/5 px-3 py-2.5 text-white outline-none focus:border-cyan-400 [color-scheme:dark]" value={purchaseDate} onChange={(e) => setPurchaseDate(e.target.value)} required/>
                  </label>
                </div>

                <label className="text-xs text-slate-400 space-y-1 block">
                  <span>Purchase Price ($)</span>
                  <input type="number" step="0.01" className="w-full rounded-xl border border-white/10 bg-white/5 px-3 py-2.5 text-white outline-none focus:border-cyan-400" placeholder={`e.g. ${currentAssetInfo.price}`} value={purchasePrice} onChange={(e) => setPurchasePrice(e.target.value)} required/>
                </label>

                <Button type="submit" disabled={addAssetMutation.isPending} className="w-full justify-center py-4 bg-cyan-400 hover:bg-cyan-500 text-slate-950 font-semibold rounded-xl">
        ...
        {portfolio.length > 0 && (<div className="mt-8 space-y-8">
            {/* KPI Section */}
            ...
            ].map((stat) => (<Card key={stat.label} className="border-white/10 bg-slate-950/65">
                ...
                <div className="h-72">
                  <ResponsiveContainer width="100%" height="100%">
                    <LineChart data={chartData}>
                      ...
                      <Line type="monotone" dataKey="value" stroke="#22d3ee" strokeWidth={3} dot={{ r: 4, fill: "#fff", stroke: "#0ea5e9", strokeWidth: 2 }}/>
                    </LineChart>
                  </ResponsiveContainer>
                </div>
              </CardContent>
            ...
            <Card className="border-white/10 bg-slate-950/65">
              ...
              <CardContent className="overflow-x-auto">
                ...
                return (<tr key={asset.id} className="hover:bg-white/5 transition">
                          ...
                          <td className="py-4 px-3 text-right">
                            {isEditing ? (<div className="flex items-center justify-end gap-1.5">
                                <input type="number" step="0.01" className="w-20 rounded border border-cyan-400 bg-slate-900 px-1 py-0.5 text-xs text-white text-right outline-none" value={editingPrice} onChange={(e) => setEditingPrice(e.target.value)} autoFocus/>
                                <Button onClick={() => handleUpdatePrice(asset.id)} size="sm" className="bg-cyan-400 hover:bg-cyan-500 text-slate-950 text-xs px-2 py-0.5 h-6">
                                  Save
                                </Button>
                              </div>) : (<div className="flex items-center justify-end gap-1.5 font-medium text-cyan-300">
                          ...
                          <td className="py-4 px-3 text-center">
                            <button onClick={() => deleteAssetMutation.mutate(asset.id)} className="rounded-lg border border-white/10 p-1.5 text-slate-400 hover:text-red-400 transition bg-white/5 hover:bg-red-500/10">
