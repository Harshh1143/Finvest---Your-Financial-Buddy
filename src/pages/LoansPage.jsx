...
export function LoansPage() {
    ...
    return (<Shell>
      <div className="mx-auto max-w-7xl px-6 py-10 lg:px-8">
        ...
        {loans.length === 0 ? (<div className="mt-12 rounded-[2rem] border border-white/10 bg-slate-950/40 p-12 text-center">
            ...
            {loans.map((loan) => (<motion.div key={loan.id} initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.25 }}>
                <Card className="border-white/10 bg-slate-950/65 flex flex-col justify-between h-full relative overflow-hidden">
                  ...
                  <CardHeader>
                    <div className="flex items-start justify-between">
                      ...
                      <div className="flex gap-1.5">
                        <button onClick={() => deleteLoanMutation.mutate(loan.id)} className="rounded-lg border border-white/10 p-1.5 text-slate-400 hover:text-red-400 transition bg-white/5 hover:bg-red-500/10">
                  ...
                  <CardContent className="space-y-4">
                    ...
                    <div className="pt-2">
                      {payingLoanId === loan.id ? (<div className="space-y-2 rounded-2xl border border-cyan-500/20 bg-cyan-950/20 p-3">
                          <p className="text-xs font-medium text-cyan-300">Make EMI Payment</p>
                          <div className="flex gap-2">
                            <input type="number" placeholder="Extra prepayment ($)" value={extraPayment} onChange={(e) => setExtraPayment(e.target.value)} className="flex-1 rounded-lg border border-white/10 bg-slate-900 px-2 py-1.5 text-xs text-white placeholder-slate-500 outline-none focus:border-cyan-400"/>
                            <Button onClick={() => handlePayEMI(loan.id)} disabled={payEMIMutation.isPending} size="sm" className="bg-cyan-400 hover:bg-cyan-500 text-slate-950 text-xs py-1">
                              Confirm
                            </Button>
                            <Button onClick={() => {
                        setPayingLoanId(null);
                        setExtraPayment("");
                    }} variant="secondary" size="sm" className="text-xs py-1">
                              Cancel
                            </Button>
                          </div>
                        </div>) : (<Button onClick={() => {
                    ...
                    }} disabled={loan.remaining <= 0} className="w-full justify-center bg-white/5 border border-white/10 text-white hover:bg-white/10 rounded-xl">
        ...
        {loans.length > 0 && (<div className="mt-8 grid gap-6 lg:grid-cols-[1fr_0.95fr]">
                ...
                <div className="h-72">
                  <ResponsiveContainer width="100%" height="100%">
                    <BarChart data={schedule}>
                      ...
                      <Bar dataKey="amount" radius={[10, 10, 0, 0]} fill="#a78bfa">
                        {schedule.map((_entry, index) => (<Cell key={`cell-${index}`} fill={COLORS[index % COLORS.length]}/>))}
                      </Bar>
                    </BarChart>
                  </ResponsiveContainer>
                </div>
            ...
            ].map(([label, value]) => (<div key={label} className="flex items-center justify-between rounded-2xl border border-white/10 bg-white/5 px-4 py-3">
                    <span className="text-slate-300">{label}</span>
                    <span className="font-semibold text-white">{value}</span>
                  </div>))}
                {totalPaid > 0 && totalPrincipal > 0 && (<div className="rounded-3xl border border-emerald-400/20 bg-emerald-500/10 p-4 text-sm text-emerald-200">
      ...
      <AddLoanModal isOpen={isAddLoanOpen} onClose={() => setIsAddLoanOpen(false)} onSuccess={async (data) => {
