...
export function DashboardPage() {
  ...
  const [depositValues, setDepositValues] = useState<Record<number, string>>({});
  ...
  const addTxMutation = useMutation({
    mutationFn: (newTx: any) => db.transactions.add(user!.id, newTx),
    onSuccess: () => {
  ...
  const setBudgetMutation = useMutation({
    mutationFn: (amount: number) => db.budgets.set(user!.id, amount),
    onSuccess: () => {
  ...
  const addGoalMutation = useMutation({
    mutationFn: (goal: any) => db.savings.add(user!.id, goal),
    onSuccess: () => {
  ...
  const addSavingsAmountMutation = useMutation({
    mutationFn: ({ goalId, amount }: { goalId: number; amount: number }) =>
      db.savings.addSavings(user!.id, goalId, amount),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["savings", user?.id] });
  ...
  const deleteGoalMutation = useMutation({
    mutationFn: (goalId: number) => db.savings.delete(user!.id, goalId),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["savings", user?.id] });
          ...
          {summary.map((item, idx) => {
            ...
            if (isSpend) {
              return (
                ...
                >
                  {isEditingBudget ? (
                    <Card className="border-white/10 bg-slate-950/65 backdrop-blur-xl h-full flex flex-col justify-between">
                        ...
                        <div className="flex gap-2">
                          <div className="relative flex-1">
                            ...
                            <input
                              ...
                              className="w-full rounded-xl border border-white/10 bg-white/5 py-1.5 pl-7 pr-3 text-sm text-white placeholder-slate-500 outline-none focus:border-cyan-400/50 focus:bg-white/10"
                            />
                          </div>
                          <button
                            type="button"
                            ...
                            className="rounded-xl bg-cyan-400 hover:bg-cyan-500 text-slate-950 p-2 font-semibold flex items-center justify-center cursor-pointer transition"
                          >
                          ...
                          <button
                            ...
                            className="rounded-xl border border-white/10 bg-transparent text-slate-400 hover:text-white p-2 flex items-center justify-center cursor-pointer transition"
                          >
                            <X className="h-4 w-4" />
                          </button>
                        </div>
                    ...
                    <Card className="border-white/10 bg-slate-950/65 backdrop-blur-xl h-full relative overflow-hidden">
                      <CardHeader className="pb-2 flex flex-row items-start justify-between space-y-0">
                        ...
                        <button
                          ...
                          className="opacity-0 group-hover:opacity-100 transition-opacity duration-200 text-slate-400 hover:text-white p-1.5 rounded-xl bg-white/5 border border-white/10 cursor-pointer flex items-center justify-center"
                      ...
                      <CardContent>
                        <div className="flex items-end justify-between">
                          ...
                          {budgetData?.monthly_budget && (
                            <span className={`flex items-center gap-0.5 rounded-full px-2.5 py-0.5 text-xs font-semibold ${
                              monthlyExpenses > budgetData.monthly_budget 
            ...
            <CardContent>
              ...
              <div className="mt-4 space-y-2">
                {finalAllocations.map((item, index) => (
                  <div
                    key={item.name}
                    className="flex items-center justify-between rounded-xl border border-white/5 bg-white/5 px-3 py-2 text-sm text-slate-300"
                  >
                ...
                {transactions.length > 0 ? (
                  transactions.slice(0, 5).map((tx) => (
                    ...
                    >
                      ...
                      <p
                        className={`font-semibold ${tx.type === "income" ? "text-emerald-400" : "text-slate-100"}`}
                      >
            ...
            <CardHeader>
              <div className="flex items-center justify-between">
                ...
                <Button 
                  ...
                  size="sm" 
                  className="rounded-xl bg-cyan-400/10 hover:bg-cyan-400/20 text-cyan-300 border border-cyan-400/20 flex items-center justify-center cursor-pointer transition"
                >
              ...
              {savings.length > 0 ? (
                savings.map((goal) => {
                  ...
                  return (
                    <div
                      key={goal.id}
                      className="rounded-2xl border border-white/5 bg-white/5 p-4 space-y-3 relative group/goal"
                    >
                      <div className="flex items-start justify-between">
                        ...
                        <div className="flex items-center gap-2">
                          ...
                          <button
                            ...
                            className="opacity-0 group-hover/goal:opacity-100 transition duration-200 text-slate-400 hover:text-red-400 p-1 rounded hover:bg-red-500/10 cursor-pointer"
                            title="Delete Goal"
                          >
                      ...
                      {!isAchieved && (
                        <div className="flex items-center gap-2 pt-1">
                          ...
                          <Button
                            ...
                            size="sm"
                            className="rounded-xl bg-cyan-400 hover:bg-cyan-500 text-slate-950 font-semibold py-1.5 px-3 cursor-pointer"
                          >
                            Deposit
                          </Button>
                        </div>
                      ...
                      {isAchieved && (
                        <p className="text-xs text-emerald-400 font-semibold bg-emerald-500/5 border border-emerald-500/10 rounded-xl py-1.5 text-center">
