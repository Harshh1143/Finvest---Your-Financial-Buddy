...
export function AuthForm() {
  ...
  return (
    <div className="flex min-h-screen items-center justify-center bg-[radial-gradient(circle_at_top_left,_rgba(56,189,248,0.12),_transparent_40%),radial-gradient(circle_at_top_right,_rgba(167,139,250,0.12),_transparent_35%),linear-gradient(135deg,_#020617,_#0f172a_32%,_#111827)] p-4 text-slate-100">
      <div className="relative w-full max-w-md overflow-hidden rounded-[2.5rem] border border-white/10 bg-slate-950/70 p-8 shadow-[0_32px_64px_-16px_rgba(0,0,0,0.6)] backdrop-blur-xl">
        ...
        {error && (
          <motion.div
            ...
            className="mb-6 rounded-2xl border border-red-500/20 bg-red-500/10 p-4 text-center text-sm text-red-300"
          >
            {error}
          </motion.div>
        )}
          ...
          {isLogin ? (
            ...
            >
              <div className="space-y-2">
                ...
                {signupErrors.name && (
                  <p className="text-xs text-red-400">{signupErrors.name.message}</p>
                )}
              </div>
        ...
        <button
          ...
          className="flex w-full items-center justify-center gap-2 rounded-2xl border border-cyan-400/20 bg-cyan-500/10 px-4 py-3.5 text-sm font-semibold text-cyan-200 transition hover:bg-cyan-500/15"
          disabled={isLoading}
        >
