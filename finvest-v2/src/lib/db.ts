...
const SEED_TRANSACTIONS = (userId: string): Transaction[] => [
  {
    id: 1,
    user_id: userId,
    amount: 5000.00,
...
const SEED_PORTFOLIO = (userId: string): PortfolioAsset[] => [
  {
    id: 1,
    user_id: userId,
    name: "Apple Inc.",
...
const SEED_BUDGET = (userId: string): Budget => ({
  id: 1,
  user_id: userId,
...
const SEED_CATEGORY_BUDGETS = (userId: string): CategoryBudget[] => [
  {
    id: 1,
    user_id: userId,
    category: "Housing",
...
const SEED_EVENTS = (userId: string): Event[] => [
  {
    id: 1,
    user_id: userId,
...
const SEED_EVENT_TRANSACTIONS = (userId: string): EventTransaction[] => [
  {
    id: 1,
    user_id: userId,
    event_id: 1,
...
const SEED_LOANS = (userId: string): Loan[] => [
  {
    id: 1,
    user_id: userId,
    name: "Home Loan",
...
const SEED_SAVINGS_GOALS = (userId: string): SavingsGoal[] => [
  {
    id: 1,
    user_id: userId,
    name: "Emergency Fund",
...
export const db = {
  auth: {
    getCurrentUser: async (): Promise<Profile | null> => {
      ...
      return getStorageItem<Profile | null>("finvest_active_user", null);
    },

    signIn: async (email: string, password: string): Promise<{ user: Profile | null; error: string | null }> => {
      if (supabase) {
      ...
      // LocalStorage Auth
      const profiles = getStorageItem<Profile[]>("finvest_profiles", []);
    ...
    signUp: async (name: string, email: string, password: string): Promise<{ user: Profile | null; error: string | null }> => {
      if (supabase) {
  ...
  transactions: {
    list: async (userId: string): Promise<Transaction[]> => {
      ...
      const list = getStorageItem<Transaction[]>("finvest_transactions", []);
      return list.filter(t => t.user_id === userId).sort((a,b) => new Date(b.date).getTime() - new Date(a.date).getTime());
    },

    add: async (userId: string, tx: Omit<Transaction, "id" | "user_id" | "created_at">): Promise<Transaction> => {
      if (supabase) {
        const { data, error } = await supabase
          .from("transactions")
    ...
    delete: async (userId: string, txId: number): Promise<boolean> => {
      if (supabase) {
  ...
  portfolio: {
    list: async (userId: string): Promise<PortfolioAsset[]> => {
      if (supabase) {
        const { data, error } = await supabase
          .from("portfolio_assets")
          .select("*")
          .eq("user_id", userId);
        if (!error && data) return data as PortfolioAsset[];
      }
      const list = getStorageItem<PortfolioAsset[]>("finvest_portfolio", []);
      return list.filter(p => p.user_id === userId);
    },

    add: async (userId: string, asset: Omit<PortfolioAsset, "id" | "user_id" | "total_value" | "total_cost" | "unrealized_pl" | "unrealized_pl_percent" | "created_at" | "updated_at">): Promise<PortfolioAsset> => {
    ...
    delete: async (userId: string, assetId: number): Promise<boolean> => {
      if (supabase) {
        const { error } = await supabase
          .from("portfolio_assets")
          .delete()
    ...
    updatePrice: async (userId: string, assetId: number, currentPrice: number): Promise<PortfolioAsset | null> => {
      if (supabase) {
  ...
  budgets: {
    get: async (userId: string): Promise<Budget | null> => {
      ...
      const list = getStorageItem<Budget[]>("finvest_budgets", []);
      return list.find(b => b.user_id === userId) || null;
    },

    set: async (userId: string, monthlyBudget: number): Promise<Budget> => {
      if (supabase) {
    ...
    listCategoryBudgets: async (userId: string): Promise<CategoryBudget[]> => {
      if (supabase) {
        const { data, error } = await supabase
          .from("category_budgets")
          .select("*")
          .eq("user_id", userId);
        if (!error && data) return data as CategoryBudget[];
      }
      const list = getStorageItem<CategoryBudget[]>("finvest_category_budgets", []);
      return list.filter(cb => cb.user_id === userId);
    },

    setCategoryBudget: async (userId: string, category: string, monthlyBudget: number): Promise<CategoryBudget> => {
      if (supabase) {
    ...
    deleteCategoryBudget: async (userId: string, category: string): Promise<boolean> => {
      if (supabase) {
        const { error } = await supabase
          .from("category_budgets")
          .delete()
  ...
  events: {
    list: async (userId: string): Promise<Event[]> => {
      if (supabase) {
        const { data, error } = await supabase
          .from("events")
          .select("*")
      ...
      const list = getStorageItem<Event[]>("finvest_events", []);
      return list.filter(e => e.user_id === userId);
    },

    add: async (userId: string, event: Omit<Event, "id" | "user_id" | "created_at">): Promise<Event> => {
      if (supabase) {
        const { data, error } = await supabase
          .from("events")
    ...
    delete: async (userId: string, eventId: number): Promise<boolean> => {
      if (supabase) {
        const { error } = await supabase
          .from("events")
          .delete()
    ...
    listTransactions: async (userId: string, eventId: number): Promise<EventTransaction[]> => {
      if (supabase) {
        const { data, error } = await supabase
          ...
          .eq("event_id", eventId)
          .order("date", { ascending: false });
        if (!error && data) return data as EventTransaction[];
      }
      const list = getStorageItem<EventTransaction[]>("finvest_event_transactions", []);
      return list.filter(et => et.user_id === userId && et.event_id === eventId);
    },

    addTransaction: async (userId: string, eventId: number, tx: Omit<EventTransaction, "id" | "user_id" | "event_id" | "created_at">): Promise<EventTransaction> => {
      if (supabase) {
        const { data, error } = await supabase
          .from("event_transactions")
    ...
    deleteTransaction: async (userId: string, txId: number): Promise<boolean> => {
      if (supabase) {
        const { error } = await supabase
          .from("event_transactions")
          .delete()
  ...
  loans: {
    list: async (userId: string): Promise<Loan[]> => {
      if (supabase) {
        const { data, error } = await supabase
          .from("loans")
          .select("*")
      ...
      const list = getStorageItem<Loan[]>("finvest_loans", []);
      return list.filter(l => l.user_id === userId);
    },

    add: async (userId: string, loan: Omit<Loan, "id" | "user_id" | "remaining" | "total_paid" | "interest_paid" | "monthly_emi" | "next_payment_date" | "created_at">): Promise<Loan> => {
      // Calculate EMI
    ...
    payEMI: async (userId: string, loanId: number, extraPayment = 0): Promise<Loan | null> => {
      if (supabase) {
    ...
    delete: async (userId: string, loanId: number): Promise<boolean> => {
      if (supabase) {
        const { error } = await supabase
          .from("loans")
          .delete()
  ...
  savings: {
    list: async (userId: string): Promise<SavingsGoal[]> => {
      if (supabase) {
        const { data, error } = await supabase
          ...
          .select("*")
          .eq("user_id", userId);
        if (!error && data) return data as SavingsGoal[];
      }
      const list = getStorageItem<SavingsGoal[]>("finvest_savings_goals", []);
      return list.filter(g => g.user_id === userId);
    },

    add: async (userId: string, goal: Omit<SavingsGoal, "id" | "user_id" | "current_amount" | "created_at">): Promise<SavingsGoal> => {
      const payload = {
        ...goal,
        user_id: userId,
        current_amount: 0.00
    ...
    addSavings: async (userId: string, goalId: number, amount: number): Promise<SavingsGoal | null> => {
      if (supabase) {
    ...
    delete: async (userId: string, goalId: number): Promise<boolean> => {
      if (supabase) {
        const { error } = await supabase
          .from("savings_goals")
          .delete()
  ...
  admin: {
    getStats: async (): Promise<AdminStats> => {
      ...
      const users = getStorageItem<Profile[]>("finvest_profiles", []);
      const transactions = getStorageItem<Transaction[]>("finvest_transactions", []);
      const portfolio = getStorageItem<PortfolioAsset[]>("finvest_portfolio", []);
    ...
    getUsers: async (): Promise<Profile[]> => {
      ...
      return getStorageItem<Profile[]>("finvest_profiles", []);
    },

    promote: async (email: string): Promise<boolean> => {
      if (supabase) {
        const { error } = await supabase
          .from("profiles")
